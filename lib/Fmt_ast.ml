...
and fmt_signature c ctx itms =
  ...
  let big_break =
    match prev with
    | Some (i', c') -> break_between c (Sig i', c'.conf) (Sig itm, c.conf)
    | None -> false
  in
  opt prev (fun _ -> fmt_if big_break "\n" $ break 1000 0)
  $ maybe_disabled c itm.psig_loc []
    @@ fun c -> fmt_signature_item c (sub_sig ~ctx itm)
...
and fmt_structure c ctx itms =
  ...
  let big_break =
    match prev with
    | Some (i_p, c_p) -> break_between c (Str i_p, c_p.conf) (Str itm, c.conf)
    | None -> false
  in
  opt prev (fun _ ->
...
and fmt_structure_item c ~last:last_item ?ext {ctx; ast= si} =
  ...
  | Pstr_value (rec_flag, bindings) ->
      ...
      let break (itmI, cI) (itmJ, cJ) =
        ...
        || break_between c (Vb itmI, cI.conf) (Vb itmJ, cJ.conf)
      in
