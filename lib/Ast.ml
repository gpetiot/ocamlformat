...
and Requires_sub_terms : sig
  ...
  let check_pat {ctx; ast= pat} =
    ...
    in
    let check_expr_function_params =
      List.exists ~f:check_expr_function_param
    in
    ...
    | Exp ctx -> (
      ...
      | Pexp_function (_, _, Pfunction_cases (cases, _, _)) | Pexp_match (_, cases) | Pexp_try (_, cases) ->
          assert (
            List.exists cases ~f:(function
              | {pc_lhs; _} when pc_lhs == pat -> true
              | _ -> false ) )
      | Pexp_for (p, _, _, _, _) -> assert (p == pat)
      | Pexp_function (params, _, _) -> assert (check_expr_function_params params) )
  ...
  let check_exp {ctx; ast= exp} =
    ...
    | Exp ctx -> (
        ...
        | Pexp_function (params, _, Pfunction_body body) ->
            assert (check_expr_function_params params || body == exp)
